...
try:
    ...
    # 点击右下的三个点
    more_button = driver.find_element(AppiumBy.XPATH, "//android.widget.RelativeLayout[@resource-id='com.netease.cloudmusic:id/moreButtonContainer1']/android.widget.FrameLayout")
    more_button.click()

    # 点击“定时关闭”
    timer_button = driver.find_element(AppiumBy.XPATH, "//android.widget.TextView[@resource-id='com.netease.cloudmusic:id/bs_list_title' and @text='定时关闭']")
    timer_button.click()

    # 选择时间为10分钟
    timer_option = driver.find_element(AppiumBy.XPATH, "//android.widget.TextView[@text='10']")
    timer_option.click()

    # 验证设置是否成功
    time.sleep(2)
    selected_time = driver.find_element(AppiumBy.XPATH, "//android.widget.TextView[@text='选择时间']")
    assert selected_time is not None, "定时关闭设置未成功"
