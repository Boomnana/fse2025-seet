...
try:
    ...
    TouchAction(driver).long_press(x=x1, y=y1).move_to(x=x2, y=y2).release().perform()
    # 滚动到锁屏显示
    # 等待并检查自动下载最新安装包元素是否存在（根据文本内容）
    wait = WebDriverWait(driver, 2)
    lyric_element = wait.until(
        EC.presence_of_element_located((By.XPATH, "//android.widget.TextView[contains(@text, '自动下载最新安装包')]"))
        # 替换为实际文本
    )
    sleep(1)
    # 点击打开
