...
class TestSwitch:
    ...
    def test_pop(self):
        ...
        try:
            text_1, text_2 = video_page.collect_display_y_o()
            print("视频收藏页视频标题--->"+text_1)
            print("我的->收藏列表视频标题--->"+text_2)
            # 使用断言来检查方法的返回值或状态
            assert text_1 == text_2, "收藏的视频在用户中心收藏列表不可见"
            print("1:收藏的视频在用户中心收藏列表可见!")
            self.driver.quit()
        except Exception as e:
            # 如果发生异常，记录异常信息，并允许 pytest 将其标记为失败
            pytest.fail(f"收藏的视频在用户中心收藏列表不可见：{str(e)}")
