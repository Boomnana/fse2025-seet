...
def test():
    ...
    # 点击入学年份
    try:
        element = driver.find_element(By.ID, "com.ss.android.article.news:id/ebb")
        element.click()
    except Exception as e:
        print("点击入学年份发生错误：", e)
    ...
    try:
        ...
        number = int(extract_numbers(element.text))
        x = width * 0.5
        y = bounds['y'] + height * 0.24
        print(x)
        print(y)
        sleep(1)
    ...
    # 点击提交
    try:
        element = driver.find_element(By.ID, "com.ss.android.article.news:id/hvz")
        element.click()
    except Exception as e:
        print("点击提交发生错误：", e)
    ...
    # 验证信息
    try:
        element = driver.find_element(By.ID, "com.ss.android.article.news:id/iz5")
        assert "广州番禺职业技术学院" in element.text, f"选择的学校：广州番禺职业技术学院与显示的学校：{element.text}不一致"
        element = driver.find_element(By.ID, "com.ss.android.article.news:id/eba")
        element.click()
        element = driver.find_element(By.ID, "com.ss.android.article.news:id/img")
        assert "2023" in element.text, f"选择的入学年份：2023与显示的入学年份：{element.text}不一致"
        print("测试通过，选择的学校和入学年份成功显示")
    except Exception as e:
        print("点击提交发生错误：", e)
