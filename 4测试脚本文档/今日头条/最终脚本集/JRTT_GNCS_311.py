...
class TestSwitch:
    ...
    def test_pop(self):
        ...
        try:
            text_1, text_2 = video_page.play_or_pause()
            print("暂停时按钮状态--->"+text_1)
            print("播放时按钮状态--->"+text_2)
            expected_value1 = '播放'
            expected_value2 = '暂停'
            # 使用断言来检查方法的返回值或状态
            assert text_1 == expected_value1 and text_2 == expected_value2, "播放/暂停功能异常"
            print("1:播放/暂停功能正常")
            self.driver.quit()
        except Exception as e:
            # 如果发生异常，记录异常信息，并允许 pytest 将其标记为失败
            pytest.fail(f"播放/暂停功能异常：{str(e)}")
