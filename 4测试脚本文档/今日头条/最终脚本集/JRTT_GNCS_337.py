...
class TestSwitch:
    ...
    def test_pop(self):
        ...
        try:
            text_1, text_2 = video_page.c_collect()
            print("收藏按钮(点击前)selected状态--->"+text_1)
            print("收藏按钮(点击后)selected状态--->"+text_2)
            expected_value1 = 'false'
            expected_value2 = 'true'
            # 使用断言来检查方法的返回值或状态
            assert text_1 == expected_value1 and text_2 == expected_value2, "收藏功能异常"
            print("1:收藏功能正常!")
            self.driver.quit()
        except Exception as e:
            # 如果发生异常，记录异常信息，并允许 pytest 将其标记为失败
            pytest.fail(f"收藏功能异常：{str(e)}")
