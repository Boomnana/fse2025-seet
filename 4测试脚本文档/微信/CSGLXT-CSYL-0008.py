...
class Test_PersonalInfo:
    ...
    def setup(self, login):
        ...
        self.name_input_loc = (By.NAME, 'name')
        self.submit_btn_loc = (By.ID, 'updSessionInfoFormBtn')
        self.ok_btn_loc = (By.CSS_SELECTOR, 'body > div.fater-model-alert > div.fater-model-alert-body > div.fater-model-alert-btns > button')
        self.msg_text_loc = (By.XPATH, '/html/body/div[7]/div[2]/div[1]')

    def test_update_name(self):
        self.browser.find_element(*self.name_input_loc).clear()
        self.browser.find_element(*self.name_input_loc).send_keys("John Doe")
        self.browser.find_element(*self.submit_btn_loc).click()
        
        # 检查是否显示了更新成功的消息
        msg_text = self.browser.find_element(*self.msg_text_loc).text
        assert "处理成功" in msg_text
        self.wait.until(expected_conditions.visibility_of_element_located(self.ok_btn_loc)).click()

        # 检查主页是否显示了新的名字
        self.wait.until(expected_conditions.visibility_of_element_located(self.center_loc)).click()
        self.wait.until(expected_conditions.visibility_of_element_located(self.info_loc)).click()
        name = self.browser.find_element(*self.name_input_loc).get_attribute('value')
